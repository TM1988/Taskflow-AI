...
} from "@/services/db/dynamicConnection"; // Added getOrganizationDatabaseConnection
...
async function getMongoProjectId(firebaseProjectId: string): Promise<string> {
  try {
    ...
    if (firebaseProjectId.length < 25) {
      ...
      console.log(
        `[Tasks getMongoProjectId] Project ID ${firebaseProjectId} looks like Firebase ID, checking for MongoDB mapping...`,
      );
      ...
      if (projectDoc.exists()) {
        ...
        if (projectData.customDbProjectId) {
          console.log(
            `[Tasks getMongoProjectId] Found MongoDB project ID: ${projectData.customDbProjectId} for Firebase ID: ${firebaseProjectId}`,
          );
    ...
    console.log(
      `[Tasks getMongoProjectId] Using original project ID: ${firebaseProjectId}`,
    );
    return firebaseProjectId;
  } catch (error) {
    console.log(
      `[Tasks getMongoProjectId] Error checking Firebase mapping:`,
      error,
    );
    return firebaseProjectId;
  }
...
export async function GET(request: NextRequest) {
  ...
  if (!projectId && !columnId && !organizationId) {
    // if no orgId, then projectId or columnId is required
    return NextResponse.json(
      {
        error:
          "Project ID, Column ID, or Organization ID is required for non-personal tasks",
      },
      { status: 400 },
    );
  }
  if (organizationId && !projectId) {
    // if orgId is present, projectId should also be present for fetching specific project tasks
  ...
  try {
    ...
    if (organizationId) {
      console.log(
        `Fetching tasks for organization: ${organizationId}, project: ${projectId || "ALL"}`,
      );
      database = await getOrganizationDatabaseConnection(organizationId);
    } else if (userId) {
      console.warn(
        `Fetching project tasks using user-specific DB for user: ${userId}, project: ${projectId}. Consider passing organizationId.`,
      );
      database = await getUserDatabaseConnection(userId);
    } else {
      console.log(
        `Fetching tasks using admin/official DB for project: ${projectId}`,
      );
    ...
    console.log(
      "Querying tasks with:",
      query,
      `in DB for org: ${organizationId || "N/A"}`,
    );
  ...
  try {
    ...
    if (projectId === "personal" && userId) {
      ...
      try {
        ...
        console.log(
          "[Tasks API POST] Connected to personal database successfully",
        );
        ...
        console.log(
          "[Tasks API POST] Inserting personal task into personalTasks collection:",
          taskData,
        );
        const result = await personalDb
          ...
          .insertOne(taskData);
        console.log(
          "[Tasks API POST] Personal task created with ID:",
    ...
    if (!organizationId || !projectId) {
      return NextResponse.json(
        {
          error:
            "Organization ID and Project ID are required for project tasks",
        },
        { status: 400 },
      );
    }
    ...
    try {
      console.log(
        `Creating task for organization: ${organizationId}, project: ${projectId}`,
      );

      // Get the correct MongoDB project ID for database operations
      const mongoProjectId = await getMongoProjectId(projectId);
      console.log(
        `[Tasks POST] Using mongoProjectId: ${mongoProjectId} for database operations`,
      );

      projectDb = await getOrganizationDatabaseConnection(organizationId);
      if (!projectDb)
        throw new Error("Organization database connection failed");
