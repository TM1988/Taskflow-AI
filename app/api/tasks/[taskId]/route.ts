...
export async function PUT(
  ...
  try {
    ...
    console.log("[PUT /api/tasks/[taskId]] Request data:", {
      taskId,
    ...
    if (data.organizationId) {
      console.log(
        `[PUT /api/tasks/[taskId]] Using organization DB for org: ${data.organizationId}`,
      );
      database = await getOrganizationDatabaseConnection(data.organizationId);
      isOrgTask = true;
    } else if (data.userId && !data.projectId) {
      // Personal task (no project)
      console.log(
        `[PUT /api/tasks/[taskId]] Using user DB for user (personal task): ${data.userId}`,
      );
      database = await getUserDatabaseConnection(data.userId);
    } else if (data.userId && data.projectId) {
      // Could be a personal project task
      console.log(
        `[PUT /api/tasks/[taskId]] Using user DB for user (personal project task): ${data.userId}, project: ${data.projectId}`,
      );
      database = await getUserDatabaseConnection(data.userId);
    } else {
      // Fallback or if context is unclear - this might need refinement
      console.warn(
        `[PUT /api/tasks/[taskId]] Falling back to admin DB (taskId: ${taskId}). Context: userId=${data.userId}, orgId=${data.organizationId}, projId=${data.projectId}`,
      );
    ...
    const collectionName = isOrgTask
      ? "tasks"
      : data.projectId === "personal" ||
          (!data.projectId && !data.organizationId)
        ? "personalTasks"
        : "tasks";
    console.log(
      `[PUT /api/tasks/[taskId]] Attempting update in collection: ${collectionName}`,
    );
    ...
    if (!existingTask) {
      ...
      console.log(
        `[PUT /api/tasks/[taskId]] Task ${taskId} not found in ${collectionName}`,
      );
    ...
    if (data.assigneeId !== undefined) updateData.assigneeId = data.assigneeId; // Added assigneeId
    if (data.assigneeName !== undefined)
      updateData.assigneeName = data.assigneeName; // Added assigneeName
    if (data.assignee !== undefined) updateData.assignee = data.assignee; // Added assignee object
    ...
    if (Object.keys(updateData).length === 0) {
      ...
      console.log(
        "[PUT /api/tasks/[taskId]] No updatable fields provided, only updating timestamp.",
      );
    }

    updateData.updatedAt = new Date();

    console.log("[PUT /api/tasks/[taskId]] Final update data:", {
    ...
    console.log("[PUT /api/tasks/[taskId]] Update result:", {
    ...
    if (result.matchedCount === 0) {
      ...
      return NextResponse.json(
        { error: "Task not found during update operation" },
        { status: 404 },
      );
    }
    ...
    const formattedTask: any = {
      ...
      dueDate: updatedTaskDoc.dueDate
        ? new Date(updatedTaskDoc.dueDate).toISOString()
        : null,
      createdAt: updatedTaskDoc.createdAt
        ? new Date(updatedTaskDoc.createdAt).toISOString()
        : null,
      updatedAt: updatedTaskDoc.updatedAt
        ? new Date(updatedTaskDoc.updatedAt).toISOString()
        : null,
    };
    ...
    if (
      !isOrgTask &&
      !formattedTask.projectId &&
      collectionName === "personalTasks"
    ) {
      formattedTask.projectId = "personal";
    }

    console.log("[PUT /api/tasks/[taskId]] Returning formatted task:", {
      id: formattedTask.id,
      assigneeId: formattedTask.assigneeId,
...
export async function DELETE(
  ...
  try {
    ...
    console.log(
      `[DELETE /api/tasks/[taskId]] Deleting task: ${taskId}, userId: ${userId}, orgId: ${organizationId}, projId: ${projectId}`,
    );
    ...
    if (organizationId) {
      console.log(
        `[DELETE /api/tasks/[taskId]] Using organization DB for org: ${organizationId}`,
      );
      ...
      // Personal task (no project)
      console.log(
        `[DELETE /api/tasks/[taskId]] Using user DB for user (personal task): ${userId}`,
      );
      database = await getUserDatabaseConnection(userId);
    } else if (userId && projectId) {
      // Could be a personal project task
      console.log(
        `[DELETE /api/tasks/[taskId]] Using user DB for user (personal project task): ${userId}, project: ${projectId}`,
      );
      database = await getUserDatabaseConnection(userId);
    } else {
      // Fallback
      console.warn(
        `[DELETE /api/tasks/[taskId]] Falling back to admin DB (taskId: ${taskId}). Context: userId=${userId}, orgId=${organizationId}, projId=${projectId}`,
      );
      database = await getAdminDb();
    }
    ...
    const collectionName = isOrgTask
      ? "tasks"
      : projectId === "personal" || (!projectId && !organizationId)
        ? "personalTasks"
        : "tasks";
    console.log(
      `[DELETE /api/tasks/[taskId]] Attempting deletion from collection: ${collectionName}`,
    );
    ...
    if (!existingTask) {
      console.log(
        `[DELETE /api/tasks/[taskId]] Task ${taskId} not found in ${collectionName}`,
      );
      return NextResponse.json({ error: "Task not found" }, { status: 404 });
    }

    console.log(
      `[DELETE /api/tasks/[taskId]] Found task to delete: ${existingTask.title}`,
    );
    ...
    if (result.deletedCount === 0) {
      return NextResponse.json(
        { error: "Task not found during deletion operation" },
        { status: 404 },
      );
    }
    ...
    try {
      ...
      console.log(
        `[DELETE /api/tasks/[taskId]] Deleted ${commentsResult.deletedCount} comments for task ${taskId}`,
      );
    } catch (error) {
      console.error(
        `[DELETE /api/tasks/[taskId]] Error deleting comments:`,
        error,
      );
